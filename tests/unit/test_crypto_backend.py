...
class TestCryptographyBackend:
    ...
    def test_chacha20_poly1305_encryption(self):
        ...
        ciphertext = self.backend.encrypt(
            "chacha20-poly1305@openssh.com", key, nonce, plaintext
        )
        assert len(ciphertext) == len(plaintext) + 16  # +16 for auth tag
        assert ciphertext != plaintext

        # Decrypt
        decrypted = self.backend.decrypt(
            "chacha20-poly1305@openssh.com", key, nonce, ciphertext
        )
        assert decrypted == plaintext
    ...
    def test_aes_gcm_encryption(self):
        ...
        # Encrypt (includes authentication tag)
        ciphertext = self.backend.encrypt(
            "aes256-gcm@openssh.com", key, nonce, plaintext
        )
        assert len(ciphertext) == len(plaintext) + 16  # +16 for auth tag
        assert ciphertext != plaintext
        ...
        # Decrypt
        decrypted = self.backend.decrypt(
            "aes256-gcm@openssh.com", key, nonce, ciphertext
        )
        assert decrypted == plaintext
