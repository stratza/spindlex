...
class TestChannelDataHandling:
    ...
    def test_handle_extended_data(self):
        ...
        self.channel._handle_extended_data(
            1, stderr_data
...
class TestChannelStreams:
    ...
    def test_exec_command_success(self):
        ...
        with patch.object(
            self.channel, "send_channel_request", return_value=True
        ) as mock_request:
            self.channel.exec_command("ls -la")

            # Verify the request was called correctly
            mock_request.assert_called_once_with(
                "exec", want_reply=True, data=b"\x00\x00\x00\x06ls -la"
            )
    ...
    def test_invoke_subsystem_success(self):
        ...
        with patch.object(
            ...
            # Verify the request was called correctly
            mock_request.assert_called_once_with(
                "subsystem", want_reply=True, data=b"\x00\x00\x00\x04sftp"
            )
