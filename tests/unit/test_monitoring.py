...
from ssh_library.logging.monitoring import (
    ...
    CryptoTimer, ProtocolAnalyzer, get_protocol_analyzer,
    PerformanceMetric, ConnectionMetrics
)
...
class TestProtocolAnalyzer:
    ...
    def test_message_size_history_bounds(self):
        ...
        assert len(sizes) <= 1000  # Should be bounded but may not be exactly 500 due to timing
    ...
    def test_performance_monitor_thread_safety(self):
        ...
        def record_metrics():
            for i in range(20):  # Reduced number to avoid deque overflow
                monitor.record_metric(f'thread_op_{threading.current_thread().ident}', 0.001 * i)
