...
class TestSSHServer(unittest.TestCase):
    ...
    def test_start_server_with_key(self, mock_transport_class):
        ...
        mock_transport_instance.start_server.assert_called_once_with(
            self.mock_key, 30.0
        )
        mock_transport_instance.set_server_interface.assert_called_once_with(
            self.server
        )
...
class TestCustomSSHServer(unittest.TestCase):
    ...
    def test_custom_exec_request_allowed(self):
        ...
        result = self.server.check_channel_exec_request(
            self.mock_channel, b"echo hello"
        )
        self.assertTrue(result)
        self.assertIn(
            ("exec", self.mock_channel, b"echo hello"), self.server.channel_calls
        )
