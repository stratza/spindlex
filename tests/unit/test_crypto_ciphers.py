...
class TestCipherSuite:
    ...
    def test_negotiate_algorithms_success(self):
        ...
        negotiated = self.cipher_suite.negotiate_algorithms(
            client_algorithms, server_algorithms
        )
        ...
        assert (
            negotiated["encryption_client_to_server"] == "chacha20-poly1305@openssh.com"
        )
        assert (
            negotiated["encryption_server_to_client"] == "chacha20-poly1305@openssh.com"
        )
    ...
    def test_algorithm_preference_order(self):
        ...
        assert kex_algs.index("curve25519-sha256") < kex_algs.index(
            "ecdh-sha2-nistp256"
        )
        assert kex_algs.index("ecdh-sha2-nistp256") < kex_algs.index(
        ...
        assert enc_algs.index("chacha20-poly1305@openssh.com") < enc_algs.index(
            "aes256-ctr"
        )
