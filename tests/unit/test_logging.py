...
from ssh_library.logging import (
    SSHLogger, get_logger, configure_logging,
    SSHFormatter, JSONFormatter, SecurityFormatter, DebugFormatter,
    SecurityHandler, PerformanceHandler,
    LogSanitizer
)
...
class TestSSHFormatter:
    ...
    def test_format_with_args(self):
        ...
        record = logging.LogRecord(
            ...
            args=('testuser', 'secret123'), exc_info=None
        )
...
class TestJSONFormatter:
    ...
    def test_format_with_sanitization(self):
        ...
        record = logging.LogRecord(
            name='test', level=logging.INFO, pathname='', lineno=0,
            msg='Login password=secret123', args=(), exc_info=None
        )
...
class TestSecurityFormatter:
    ...
    def test_format_security_event(self):
        ...
        record = logging.LogRecord(
            name='test', level=logging.INFO, pathname='', lineno=0,
            msg='Authentication failed', args=(), exc_info=None
        )
        record.client_ip = '192.168.1.100'
        
        result = self.formatter.format(record)
        assert 'SECURITY.INFO' in result
        assert '192.168.1.100' in result or '192.168.1.***' in result  # IP may or may not be sanitized in this context
    ...
    def test_format_without_client_ip(self):
        ...
        record = logging.LogRecord(
            name='test', level=logging.INFO, pathname='', lineno=0,
            msg='Security event', args=(), exc_info=None
        )
    ...
    def test_configure_with_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                configure_logging(
                    level='INFO',
                    output_file=main_log,
                    security_file=security_log
                )
                ...
                for logger_name in ['ssh_library', 'ssh_library.security', 'ssh_library.performance']:
