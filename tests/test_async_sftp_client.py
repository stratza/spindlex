...
async def test_async_sftp_client_init(mock_channel):
    ...
    with patch.object(client, "_send_message", new_callable=AsyncMock) as mock_send:
        with patch.object(client, "_recv_message", new_callable=AsyncMock) as mock_recv:
            mock_recv.side_effect = [SFTPVersionMessage(3, {}), asyncio.CancelledError()]
...
async def test_async_sftp_client_remove(async_sftp_client):
    with patch.object(async_sftp_client, "_wait_for_response", new_callable=AsyncMock) as mock_wait:
...
async def test_async_sftp_client_listdir(async_sftp_client):
    with patch.object(async_sftp_client, "_opendir", new_callable=AsyncMock) as mock_opendir:
        with patch.object(async_sftp_client, "_readdir", new_callable=AsyncMock) as mock_readdir:
            with patch.object(async_sftp_client, "_close", new_callable=AsyncMock) as mock_close:
