...
class BenchmarkResult:
    ...
    def __str__(self):
        return (f"{self.name}: {self.mean:.4f}±{self.stdev:.4f} {self.unit} "
                f"(n={self.count}, min={self.min_time:.4f}, max={self.max_time:.4f})")


def benchmark_function(func, iterations: int = 100, warmup: int = 10) -> BenchmarkResult:
    ...
    def test_encryption_performance(self):
        ...
        for size in test_sizes:
            ...
            def encrypt_chacha20():
                cipher = backend.create_cipher('chacha20-poly1305@openssh.com', key, nonce)
