...
class PerformanceProfiler:
    ...
    def print_report(self):
        ...
        for name in sorted(self.results.keys()):
            ...
            if stats['count'] > 1:
                print(f"  95th:  {sorted(self.results[name])[int(0.95 * len(self.results[name]))]:.4f}s")
    ...
    def test_command_execution_scaling(self, mock_ssh_server, profiler):
        ...
        try:
            ...
            def exec_command(cmd_id):
                stdin, stdout, stderr = client.exec_command(f'echo "concurrent_{cmd_id}"')
                return stdout.read()
    ...
    def test_file_transfer_performance(self, sftp_test_setup, profiler):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for size in file_sizes:
                ...
                print(f"SFTP {size}B - Upload: {upload_throughput:.2f} MB/s, Download: {download_throughput:.2f} MB/s")
