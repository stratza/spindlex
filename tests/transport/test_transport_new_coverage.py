...
from spindlex.protocol.messages import (
    Message, UserAuthFailureMessage, UserAuthSuccessMessage, ChannelDataMessage
)
from spindlex.exceptions import AuthenticationException
from spindlex.protocol.constants import (
    MSG_USERAUTH_SUCCESS, MSG_USERAUTH_FAILURE, MSG_CHANNEL_DATA
)
...
def test_handle_auth_response_message_partial_success():
    sock = MagicMock()
    transport = Transport(sock)
    msg = UserAuthFailureMessage(authentications=["publickey", "password"], partial_success=True)
    with pytest.raises(AuthenticationException, match="Partial success - additional methods required: publickey, password"):
...
def test_handle_auth_response_message_reunpack_path():
    ...
    from spindlex.protocol.utils import write_string, write_boolean
    data = bytearray()
...
def test_handle_channel_message_data_reunpack_path():
    ...
    from spindlex.protocol.utils import write_uint32, write_string
    data = bytearray()
...
async def test_async_expect_message_with_optional_channel_id():
    ...
    with patch('asyncio.get_event_loop', return_value=MagicMock()):
        ...
        with patch.object(AsyncTransport, '_recv_message_async', return_value=msg):
            ...
            transport._message_queue.append(msg)
            res = await transport._expect_message_async(MSG_USERAUTH_SUCCESS, channel_id=None)
            assert res == msg
