...
def test_chacha20_poly1305_roundtrip():
    ...
    # Encrypt
    encrypted = backend.chacha20_poly1305_encrypt(
        key, seq_num, length_bytes, body_bytes
    )
    ...
    # Decrypt body
    dec_body = backend.chacha20_poly1305_decrypt_body(
        key, seq_num, enc_length, enc_body, tag
    )
...
def test_chacha20_poly1305_invalid_tag():
    ...
    with pytest.raises(CryptoException, match="Poly1305 authentication failed"):
        backend.chacha20_poly1305_decrypt_body(
            key, seq_num, enc_length, enc_body, bytes(corrupt_tag)
        )
...
def test_chacha20_poly1305_wrong_seq():
    ...
    with pytest.raises(CryptoException, match="Poly1305 authentication failed"):
        backend.chacha20_poly1305_decrypt_body(
            key, seq_num + 1, enc_length, enc_body, tag
        )
