...
class IntegrationTestRunner:
    ...
    def _run_test_category(self, category: Dict) -> bool:
        ...
        # Add output options
        junit_file = self.output_dir / f"{category['name'].lower().replace(' ', '_')}_junit.xml"
        cmd.extend(['--junit-xml', str(junit_file)])
        
        # Add coverage if available
        try:
            import coverage
            cov_file = self.output_dir / f"{category['name'].lower().replace(' ', '_')}_coverage.xml"
...
def main():
    ...
    parser.add_argument('--openssh', action='store_true',
                       help='Include OpenSSH interoperability tests')
