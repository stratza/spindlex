...
def test_chacha20_local_integration():
    ...
    try:
        ...
        with SSHClient() as client:
            ...
            client.connect("127.0.0.1", port=port, username="testuser", password="password123", timeout=10)
            
            assert client.get_transport().active
            assert client.get_transport().authenticated
            assert client.get_transport()._cipher_out_active == "chacha20-poly1305@openssh.com"
            assert client.get_transport()._cipher_in_active == "chacha20-poly1305@openssh.com"
...
async def test_chacha20_async_local_integration():
    ...
    try:
        ...
        async with AsyncSSHClient() as client:
            client.set_host_key_storage(HostKeyStorage("/dev/null"))
            client.set_missing_host_key_policy(AutoAddPolicy(accept_risk=True))
            await client.connect("127.0.0.1", port=port, username="testuser", password="password123", timeout=10)
