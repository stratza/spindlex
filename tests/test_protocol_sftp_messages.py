...
from spindlex.protocol.sftp_messages import (
    SFTPMessage, SFTPAttributes, SFTPInitMessage, SFTPVersionMessage,
    SFTPStatusMessage, SFTPOpenMessage, SFTPHandleMessage, SFTPCloseMessage,
    SFTPReadMessage, SFTPWriteMessage, SFTPDataMessage, SFTPStatMessage,
    ...
    SFTPLinkMessage, SFTPExtendedMessage, SFTPExtendedReplyMessage
)
...
def test_sftp_status_message():
    msg = SFTPStatusMessage(request_id=1, status_code=SSH_FX_OK, message="OK", language="en")
    data = msg.pack()
    msg2 = SFTPMessage.unpack(data)
...
def test_sftp_open_message():
    attrs = SFTPAttributes()
    msg = SFTPOpenMessage(request_id=1, filename="test.txt", pflags=SSH_FXF_READ, attrs=attrs)
    msg.validate()
    data = msg.pack()
    msg2 = SFTPMessage.unpack(data)
...
def test_sftp_extended_message():
    msg = SFTPExtendedMessage(request_id=1, extended_request="req", extended_data=b"data")
    data = msg.pack()
    msg2 = SFTPMessage.unpack(data)
